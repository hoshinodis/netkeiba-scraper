...
object RowExtractor {
  ...
  private[this] def parseHtml(html: String) = {
  ...
  def extract()(implicit s: DBSession): Unit = {
    ...
    while (i < usefulFiles.length) {
      ...
      val fieldScore = Try{
        val rt2Lines = {
          lines.dropWhile(!_.contains("class=\"result_table_02\""))
      ...
      val rt1Lines = {
        lines.dropWhile(s => !s.contains("race_table_01"))
      ...
      val payoffLines = {
        lines.dropWhile(!_.contains("<dl class=\"pay_block\">"))
          .takeWhile(!_.contains("</dl>"))
      }
      val payoffHtml =
        payoffLines.mkString + "</dl>"
      val payoffs = {
        parseHtml(payoffHtml).\\("tr").flatMap { tr =>
